...
start() ->
    ...
    Colors = lists:map(fun binary_to_list/1, binary:split(ColorsBinary, [<<"\n">>], [global, trim])),

    %% Creazione dei nodi
    io:format("Sono start_system e inizio a creare nodi.~n"),
    Nodes = [{X, Y, node:create_node({X, Y}, lists:nth(Y, lists:nth(X, Colors)), self())} || 
...
find_neighbors(X, Y, N, M, NodeMap) ->
    NeighborCoords = [{X+DX, Y+DY} || DX <- [-1, 0, 1], DY <- [-1, 0, 1],
                      ...
                      X+DX >= 1, X+DX =< N, Y+DY >= 1, Y+DY =< M],
    [Pid || {NX, NY} <- NeighborCoords, 
...
flatten_nodes(Nodes) ->
    ...
    case lists:any(fun({_, _, Pid}) -> is_pid(Pid); (_) -> false end, Nodes) of
        true -> Nodes;  %% Se è già appiattito, restituisci Nodes così com'è
        false -> [{X, Y, Pid} || {X, Y, {_, _, Pid}} <- Nodes]  %% Esegui il flattening se necessario
    end.